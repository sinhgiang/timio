...
interface Props {
  initialRequests: EarlyLeaveRequest[];
}

const STATUS_CONFIG: Record<ELStatus, { label: string; cls: string }> = {
...
export default function EarlyLeaveClient({ initialRequests }: Props) {
  ...
  const [filter, setFilter] = useState<ELStatus | "all">("pending");
  ...
  const filterButtons: { key: ELStatus | "all"; label: string }[] = [
                ...
                {displayed.map((r) => (
                  <>
                    <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      {/* Leave time */}
                      <td className="px-4 py-3">
                        <span className="font-semibold text-blue-600">{r.leaveTime}</span>
                      </td>
                        ...
                        <td colSpan={6} className="px-4 py-3">
                          <div className="flex items-center gap-3 flex-wrap">
                            ...
                            <input
                              ...
                              placeholder="Ví dụ: Cần có mặt cuộc họp cuối giờ..."
