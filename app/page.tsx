...
export default function HomePage({ searchParams }: { searchParams?: { aff?: string } }) {
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      {/* ── APP NHÂN VIÊN + ĐIỂM TIN CẬY (worker-owned — điểm mới) ── */}
      <section id="app-nhan-vien" className="py-20 px-4 bg-gradient-to-b from-white to-blue-50/50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-12">
            <span className="inline-flex items-center gap-1.5 bg-blue-100 text-blue-700 border border-blue-200 text-xs font-bold px-3 py-1 rounded-full mb-4">
              <Sparkles size={13} /> MỚI · APP NHÂN VIÊN
            </span>
            <h2 className="text-3xl sm:text-4xl font-extrabold text-gray-900 mb-4 leading-tight">
              Mỗi nhân viên có “app của riêng mình”
            </h2>
            <p className="text-gray-500 text-lg">
              Không chỉ chấm công cho sếp — nhân viên có <strong className="text-gray-700">hồ sơ riêng, tự tạo đơn từ, xem lương minh bạch</strong>, và một <strong className="text-gray-700">điểm tin cậy mang đi được</strong>.
            </p>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              <ul className="space-y-4">
                {[
                  [Fingerprint, "Hồ sơ do nhân viên SỞ HỮU", "Ảnh, kinh nghiệm, kỹ năng — nhân viên tự làm chủ, mang theo khi đổi chỗ làm. Riêng tư mặc định, tự chọn chia sẻ."],
                  [FileText, "Tự tạo đơn từ ngay trong app", "Xin nghỉ phép, về sớm, sửa chấm công, tăng ca — công ty duyệt, có thông báo. Có dữ chứng rõ ràng thay vì nhắn Zalo."],
                  [Receipt, "Xem lương & chấm công của mình", "Phiếu lương từng kỳ, đi muộn/về sớm/tăng ca, bị trừ vì gì — minh bạch nên tới kỳ lương không cãi nhau."],
                  [BadgeCheck, "Điểm tin cậy di động", "Từ chấm công thật (đúng giờ, chuyên cần, gắn bó). Nhân viên mang đi xin việc / ứng lương tốt hơn — có lợi cho chính họ."],
                  [Bell, "Thông báo như Facebook/Zalo", "Duyệt phép, trả lương, nhà tuyển dụng quan tâm — chuông báo tận tay nhân viên."],
                ].map(([Icon, t, d]) => {
              ...
              <div className="flex flex-wrap gap-3 mt-7">
                ...
                <a href="/nhanvien" className="inline-flex items-center gap-2 border border-gray-300 text-gray-700 font-semibold px-6 py-3 rounded-xl hover:bg-gray-50 transition-colors text-sm">Xem app nhân viên</a>
              </div>
            </div>

            {/* Mock: thẻ hồ sơ nhân viên + điểm tin cậy */}
            <div className="relative">
              <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 p-5 max-w-sm mx-auto">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-indigo-700 flex items-center justify-center text-white font-bold">NV</div>
                  <div className="flex-1"><p className="font-bold text-gray-900 flex items-center gap-1">Nguyễn Văn Hải <BadgeCheck size={15} className="text-blue-500" /></p><p className="text-xs text-gray-500">Hướng dẫn viên · Công ty AZLAB</p></div>
                </div>
                <div className="rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 p-4 text-white mb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] font-semibold text-blue-100">Điểm tin cậy Timio</span>
                    <span className="text-[10px] font-bold bg-white/20 px-2 py-0.5 rounded-full">Bạc · Đáng tin</span>
                  </div>
                  <p className="text-3xl font-extrabold leading-none">80<span className="text-sm text-blue-200">/100</span></p>
          ...
          {/* Tuyển dụng — để nhẹ, một dòng (chưa đẩy mạnh trên trang chủ) */}
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-x-3 gap-y-2 text-center bg-white border border-gray-100 rounded-2xl px-5 py-4">
            <p className="text-sm text-gray-600"><Briefcase size={15} className="inline text-blue-600 mr-1" /> Timio cũng có <strong className="text-gray-800">Tuyển dụng + Kho ứng viên xác thực</strong> (đăng tin miễn phí) — đang mở rộng dần.</p>
            <a href="/viec-lam" className="text-sm font-semibold text-blue-600 hover:underline shrink-0">Xem trang việc làm →</a>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              {
                icon: Briefcase,
                title: "Tuyển dụng + Kho ứng viên xác thực",
                desc: "Đăng tin miễn phí, sàng lọc bằng AI theo tiêu chí, tuyển 1 chạm thành nhân viên. Kho ứng viên xác thực bằng dữ liệu chấm công. (Đang mở rộng dần.)",
                tag: "Business",
              },
            ].map((f) => (
